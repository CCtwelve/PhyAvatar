...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Comma-separated spatial subdirectories to process (e.g., 00,01,02).",
    )
    parser.add_argument(
        "--num_keypoints",
        type=int,
    ...
    normalized_subdirs = normalize_process_subdirs(args.process_subdirs)

    def collect_image_paths(images_dir, subdirs):
        patterns = ("**/*.jpg", "**/*.webp")
        collected = []
        for pattern in patterns:
            collected.extend(list_selected_image_paths(images_dir, subdirs, pattern))
        return sorted(collected)

    image_paths = collect_image_paths(args.images_dir, normalized_subdirs)
    if not image_paths:
        subdir_msg = f" under subdirectories {normalized_subdirs}" if normalized_subdirs else ""
        raise FileNotFoundError(f"No images (*.jpg/*.webp) found in {args.images_dir}{subdir_msg}.")

    if args.fmasks_dir is not None and args.fmasks_dir != "None":
        fmask_paths = []
        missing_masks = []
        for image_path in image_paths:
            rel_path = osp.relpath(image_path, args.images_dir)
            rel_stem, _ = osp.splitext(rel_path)
            candidate_png = osp.join(args.fmasks_dir, rel_stem + ".png")
            candidate_jpg = osp.join(args.fmasks_dir, rel_stem + ".jpg")
            if osp.exists(candidate_png):
                fmask_paths.append(candidate_png)
            elif osp.exists(candidate_jpg):
                fmask_paths.append(candidate_jpg)
            else:
                missing_masks.append(candidate_png)
                fmask_paths.append(candidate_png)
        if missing_masks:
            missing_previews = "\n  ".join(missing_masks[:5])
            raise FileNotFoundError(
                f"Missing {len(missing_masks)} fmask files corresponding to the selected images. "
                f"Examples:\n  {missing_previews}"
            )
    else:
        fmask_paths = None

    selected_info = f" subdirs {normalized_subdirs}" if normalized_subdirs else ""
    if fmask_paths is None:
        print(f"[vis_pose] Found {len(image_paths)} images{selected_info}.")
    else:
        print(f"[vis_pose] Found {len(image_paths)} images and {len(fmask_paths)} masks{selected_info}.")
