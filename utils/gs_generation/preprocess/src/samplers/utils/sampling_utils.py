...
def load_pipelines(
    ...
    try:
        ...
        # Create symlinks for variant files so diffusers can find them
        # For fp16, files are named *.fp16.safetensors, but diffusers expects *.safetensors
        if torch_dtype == torch.float16:
            import glob
            for variant_file in glob.glob(osp.join(model_dir, "**", "*.fp16.safetensors"), recursive=True):
                expected_file = variant_file.replace(".fp16.safetensors", ".safetensors")
                if not osp.exists(expected_file):
                    os.symlink(osp.basename(variant_file), expected_file)
                    log.info(f"Created symlink: {expected_file} -> {osp.basename(variant_file)}")
