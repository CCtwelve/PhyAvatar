...
class SpaTemDataset(Dataset):
    def __init__(
        ...
        height: int = 1024,
        width: int = 1024,
        has_gt_target: bool = False # if False, use skeleton as image and fmask for target samples
    ):
    ...
    def get_item(
        ...
        for i, label in enumerate(labels):
            ...
            # read data
            print("has_gt_target",self.has_gt_target,(not self.has_gt_target),spa_label not in input_spa_labels)
            ...
            if not self.has_gt_target and spa_label not in input_spa_labels:
                ...
                print("skeletion spa_label not in input_spa_labels",spa_label, input_spa_labels)
                image = skeleton
                fmask = skeleton_to_mask(skeleton).convert("L")
            else:
                print("image spa_label not in input_spa_labels",spa_label, input_spa_labels)
                image = Image.open(image_path)
                fmask = Image.open(fmask_path).convert("L")
