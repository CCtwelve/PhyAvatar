...
def build_output_path(image_path, src_root, dst_root, new_ext):
    rel_path = os.path.relpath(image_path, src_root)
    base, _ = os.path.splitext(rel_path)
    return os.path.join(dst_root, base + new_ext)
...
def remove_background(
    ...
    # prepare paths
    normalized_image_ext = image_ext if image_ext.startswith(".") else f".{image_ext}"
    normalized_mask_ext = mask_ext if mask_ext.startswith(".") else f".{mask_ext}"

    image_paths = []
    for ext in {normalized_image_ext.lower(), ".webp"}:
        image_paths.extend(glob(f"{images_dir}/**/*{ext}", recursive=True))
    image_paths = sorted(set(image_paths))

    fmask_paths = [
        build_output_path(path, images_dir, out_fmasks_dir, normalized_mask_ext) for path in image_paths
    ]
    if out_images_alpha_dir is not None:
        image_alpha_paths = [
            build_output_path(path, images_dir, out_images_alpha_dir, ".png") for path in image_paths
        ]
    else:
