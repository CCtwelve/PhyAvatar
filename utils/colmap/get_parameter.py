...
from utils.camera_data import CameraData, read_calibration_csv
import os
...
def main():
    ...
    parser.add_argument(
        "--csv", type=Path, default=Path("data/calibration.csv"),
        help="Path to input calibration CSV file"
    )
    parser.add_argument(
        "--output_dir", type=Path, default=Path("data/cam_txt_25"),
        help="Output directory for COLMAP files"
    )
    parser.add_argument(
        "--image_path", type=Path, default=Path("data/multipleview/25"),
