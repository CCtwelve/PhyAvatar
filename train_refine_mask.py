...
from utils.log_util import save_images, create_log_dir, initialize_logging, cleanup_logging, log_training_metrics, append_to_log
...
class TrainRunner():
    def __init__(self,**kwargs):
        ...
        self.dataset = AvatarDataset(**train_data,dataset=self.tps.dataset,use_depth=getattr(self.args, "use_depth", False))
        ...
        elif self.args.train_type == "zero":
            size = self.args.point_size
            init_points = torch.zeros(size,3)
            radius = self.dataset.get_radius()
            self.gaussians.create_from_pcd(init_points,torch.rand_like(init_points),spatial_lr_scale = radius)

        elif self.args.train_type == "colmap" or self.args.train_type == "smpl":
            radius = self.dataset.get_radius()
        ...
        elif self.args.train_type == "LHM" or self.args.train_type == "Diffuman4D":
            downsample_size = getattr(self.args, "downsample", None)
            self.gaussians.load_ply(self.args.ref_gs_path,downsample_size=downsample_size,downsample_path=self.save_path) 
    ...
    def run(self):
        ...
        for iteration in range(self.first_iter, self.opt.iterations + 1):
            ...
            if getattr(self.opt, 'gaussian_dropout_rate', 0) > 0:
                num_gaussians = self.gaussians.get_xyz.shape[0]
                mask = (torch.rand(num_gaussians, device=config.device) > self.opt.gaussian_dropout_rate).unsqueeze(-1)
                opacities = opacities * mask
                
            if getattr(self.opt, 'opacity_noise_strength', 0) > 0:
                opacities_to_modify = render_kwargs.get('opacities', self.gaussians.get_opacity)

                noise = 1.0 + (torch.randn_like(opacities_to_modify) * self.opt.opacity_noise_strength)

                opacities = opacities * noise

            render_kwargs['opacities'] = torch.clamp(opacities, 0.0, 1.0)
                
            render_result = render(self.gaussians,self.pipe,Camera.from_c2w(items["extr"],items["intr"],items["img_h"],items["img_w"],items["img_name"]),self.background_color,**render_kwargs)
            ...
            if getattr(self.args, "use_depth", False):
                depth_l1_weight = get_expon_lr_func(self.opt.depth_l1_weight_init, self.opt.depth_l1_weight_final, max_steps=iteration)
            if getattr(self.args, "use_depth", False) and depth_l1_weight(iteration) > 0:
                Ll1depth_pure = torch.tensor(0.0, device=config.device)
                gt_depth = items["depth_img"].float().unsqueeze(0)
                Ll1depth_pure = torch.abs((gt_depth  - render_depth) * gt_mask).mean()
                Ll1depth = depth_l1_weight(iteration) * Ll1depth_pure 
                loss += Ll1depth
                Ll1depth = Ll1depth.item()
            else:
                Ll1depth = torch.tensor(0.0, device=config.device)
            dict_loss["Ll1depth"] = Ll1depth
    ...
    def training_report(self, iteration,loss,dict_loss):
        ...
        if iteration % self.opt.log_save_interval == 0:
            ...
            for idx in range(views_num_train):    
                ...
                if getattr(self.args, "use_depth", False):
                    gt_depth = items["depth_img"].float().unsqueeze(0)
                    render_depth = render_result["render_depth"]
                    images["render_depth"] = render_depth
                    images["gt_depth"] = gt_depth
