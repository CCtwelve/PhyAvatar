...
class GaussianModel:
    ...
    def save_ply(self, path,ply_name,quantised=False, half_float=False):
        if quantised:
            ply_name += "_quantised"
        if half_float:
            ply_name += "_half"
        ...
        for sh_degree in range(self.max_sh_degree + 1):
            ...
            degrees_mask = (self.degrees == sh_degree).squeeze()

            #  Position is not quantised
            if half_float:
                xyz = self.xyz[degrees_mask].detach().cpu().half().view(dtype=torch.int16).numpy()
            else:
                xyz = self.xyz[degrees_mask].detach().cpu().numpy()
            ...
            if self.variable_sh_bands:
                f_rest = features_rest[sh_degree][:, :coeffs_num].detach().transpose(1, 2).flatten(
                    start_dim=1).contiguous().cpu().numpy()
            else:
                f_rest = features_rest[degrees_mask][:, :coeffs_num].detach().transpose(1, 2).flatten(
                    start_dim=1).contiguous().cpu().numpy()
    ...
    def densify_and_prune(self,args,iter, max_grad, min_opacity, extent, max_screen_size, radii,densification_statistics_dict, store_grads=False):
        grads = self.xyz_gradient_accum / self.denom
        grads[grads.isnan()] = 0.0

        self.tmp_radii = radii
        n_points_cloned = self.densify_and_clone(grads, max_grad, extent, store_grads=store_grads)  # 对梯度高于 max_grad 且尺寸较小的高斯点进行克隆，在周围增加新点以捕捉细节
        n_points_split = self.densify_and_split(grads, max_grad, extent, store_grads=store_grads)  # 对梯度高于 max_grad 且尺寸较大的高斯点进行分裂，将其拆分为多个小高斯点
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2, store_grads=False):
        ...
        new_degrees = self.degrees[selected_pts_mask].repeat(N,1)
        ...
        self.prune_points(prune_filter, store_grads, store_grads=store_grads)
        return n_points_split

    def prune_optimizer(self, mask, store_grads=False):
        optimizable_tensors = {}
    ...
    def prune_points(self, mask, store_grads = False):
        ...
        optimizable_tensors = self.prune_optimizer(valid_points_mask, store_grads=store_grads)
        ...
        self.degrees = self.degrees[valid_points_mask]
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent, store_grads=False):
        ...
        new_degrees = self.degrees[selected_pts_mask]

        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling,
                                   new_rotation,  new_degrees, store_grads=store_grads)
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation,new_degrees, store_grads=False):
        ...
        self.scaling = optimizable_tensors["scaling"]
        self.rotation = optimizable_tensors["rotation"]
        self.degrees = torch.cat((self.degrees, new_degrees), dim=0)
